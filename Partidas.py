...
class Persistencia:
    ...
    def guarda_slot(self, slot: int, escenario, jugador):
        """Guarda en el slot (0..max_slots-1)."""
        try:
            datos = {'escenario': escenario, 'jugador': jugador}
            nombre = self.nombre_slot(slot)
            with open(nombre, 'wb') as fos:
            ...
            print(f"Error al guardar en slot {slot+1}: {e}")
            return False
    ...
    def rescatar_slot(self, slot: int):
        ...
        try:
            ...
            print(f"Error al rescatar slot {slot+1}: {e}")
            return None, None

    def listar_slots(self):
        """Devuelve lista de metadatos (None o dict con filename/fecha)."""
        return self._leer_index()

    def borrar_slot(self, slot: int):
        index = self._leer_index()
        if 0 <= slot < self.max_slots and index[slot]:
            fname = index[slot].get('filename')
            try:
...
class Controlador:
    def __init__(self):
        ...
        # índice rotativo para guardado automático
        self._next_slot = 0

    def guardar_partida_auto(self, escenario, jugador):
        """Guarda en el primer slot vacío; si todos llenos, sobrescribe en rotación."""
        ...
        if slot_libre is None:
            ...
            slot = slot_libre
        ok = self.persistencia.guarda_slot(slot, escenario, jugador)
            ...
            for evento in pygame.event.get():
                if evento.type == pygame.QUIT:
                    ...
                    if evento.key == pygame.K_w:
                        ...
                        # guardado automático en primer slot libre o rotativo
                        ok, slot = self.controlador.guardar_partida_auto(self.escenario, self.jugador)
                        if ok:
                            self.mensajes.append(f"🎉 Partida guardada en slot {slot + 1}.")
                        else:
...
# ------------------------- MENU INICIAL Y CONFIGURACIÓN (con P: Partidas guardadas) -------------------------
...
def menu_partidas_guardadas(controlador: Controlador):
    """Muestra un menú simple con los 3 slots y devuelve el slot seleccionado (0..2) o None si vuelve.
       IMPORTANT: NO debe cerrar el display global; solo ajusta el modo para mostrar el submenu.
    """
    # NO llamamos a pygame.init() ni pygame.display.quit() aquí para no invalidar la surface de quien llamó.
    ancho = 600
    alto = 300
    ...
    while seleccion is None:
        ...
        try:
            ...
            # si por alguna razón el display fue cerrado, re-crearlo
            pygame.display.init()
            pantalla = pygame.display.set_mode((ancho, alto))
            pantalla.fill((10, 10, 10))

        pantalla.blit(fuente.render("Partidas guardadas (presiona 1,2,3 para cargar, ESC para volver)", True, (255, 255, 255)), (20, 20))
        for i in range(3):
            y = 80 + i * 50
            meta = index[i] if index and i < len(index) else None
            if meta:
                fecha = meta.get('fecha', 'sin fecha')
                pantalla.blit(fuente.render(f"{i+1}) Slot {i+1} - Guardada: {fecha}", True, (200, 200, 200)), (40, y))
            else:
                pantalla.blit(fuente.render(f"{i+1}) Slot {i+1} - Vacío", True, (150, 150, 150)), (40, y))

        if info_msg:
            pantalla.blit(fuente.render(info_msg, True, (255, 100, 100)), (20, alto - 40))

        pygame.display.flip()
        clock.tick(30)

    # NO cerramos la display aquí; devolvemos la selección al menú padre
    return seleccion
...
def menu_inicial(controlador: Controlador):
    ...
    while seleccion is None:
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_n:  # Nueva partida
                    seleccion = 'n'
                elif evento.key == pygame.K_l:  # Cargar última (compatibilidad)
                    seleccion = 'l'
                elif evento.key == pygame.K_p:  # Partidas guardadas
        ...
        try:
            ...
            # si el display fue cerrado por accidente, volver a crearla
            pygame.display.init()
            pantalla = pygame.display.set_mode((ancho, alto))
            pantalla.fill((20, 20, 20))

        pantalla.blit(fuente_titulo.render("GeoZ4 - Simulación Zombie", True, (255, 255, 255)), (40, 30))
        pantalla.blit(fuente.render("N - Nueva partida", True, (200, 200, 200)), (40, 110))
        pantalla.blit(fuente.render("L - Cargar partida (último guardado por compatibilidad)", True, (200, 200, 200)), (40, 150))
        pantalla.blit(fuente.render("P - Partidas guardadas (ver 3 slots y elegir)", True, (200, 200, 200)), (40, 190))
        pantalla.blit(fuente.render("Q - Salir", True, (200, 200, 200)), (40, 230))

        pantalla.blit(fuente.render("Controles: W A S D, E interactuar, G guardar (guarda en primer slot libre)", True, (160, 160, 160)), (40, 320))

        pygame.display.flip()
        clock.tick(30)

    # cerramos la ventana del menú antes de seguir (opcional; el juego la volverá a crear)
    pygame.display.quit()
    return seleccion, seleccion_slot
...
if __name__ == '__main__':
    ...
    if opcion == 'l':
        # compatibilidad: cargar último guardado (slot más reciente)
        index = controlador_global.listar_saves()
        # buscar slot más reciente por fecha
        most_recent = None
        recent_slot = None
        ...
        if recent_slot is not None:
            ...
            if esc is not None and jug is not None:
                ...
                print("No se pudo cargar la partida. Creando nueva partida.")
                escenario = crear_escenario_predeterminado()
        else:
            print("No hay partidas guardadas. Creando nueva partida.")
            escenario = crear_escenario_predeterminado()
        ...
        if esc is not None and jug is not None:
            ...
            print(f"No se encontró partida válida en slot {opcion_slot + 1}. Creando nueva partida.")
            escenario = crear_escenario_predeterminado()
